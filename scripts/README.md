...
| The dashboard starts, but **no resource ever leaves `Waiting`** and the dashboard shows `postgres-password` as `ValueMissing` | The AppHost is running outside the Development environment, so `dotnet user-secrets` was never added to configuration — and every parameter marked `secret: true` comes from there. | Check that `src/…AppHost/Properties/launchSettings.json` exists and sets `DOTNET_ENVIRONMENT=Development`. This fails almost silently: the AppHost prints a dashboard URL and nothing else. |
| `The launch profile "(Default)" could not be applied.` followed by `The JSON value could not be converted to System.String` | A non-string value inside `environmentVariables` in a `launchSettings.json` — a `"//"` comment array is the usual culprit. | Put comments at the top level of the file, not inside `environmentVariables`. Until you do, `dotnet run` continues with **no profile at all**, which looks exactly like the row above. |
| `option '-p, --port <port>' argument '${PORT:-3000}' is invalid` | A POSIX shell expansion in a `package.json` script. pnpm on Windows passes it through literally. | Scripts must be shell-agnostic. Next reads `PORT` from the environment on its own, and the AppHost appends its own `-p`, so the script names no port. |
